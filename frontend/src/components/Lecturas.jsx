...
const METRICA_CAMPO_REC = {
  humedad_suelo: 'humedad_suelo', temperatura_aire: 'temperatura_aire',
  ph_suelo: 'ph_suelo', lluvia: 'lluvia',
  velocidad_viento: 'velocidad_viento', humedad_aire: 'humedad_aire',
}

const VENTANAS = [
  { id: '30m', label: 'Última media hora' },
...
export default function Lecturas() {
  ...
  const loadSensores = async () => {
    ...
    try {
      ...
      const filtrados = filtroEstado === 'todos' ? todos : todos.filter(d => d.estado === filtroEstado)
      const mapeados  = filtrados.map(d => {
        ...
        return { id: d.id_logico, label: d.id_logico, metric: metrica, color: config.color, unit: config.unit, min: config.min, max: config.max, estado: d.estado, tipo_id: d.tipo_dispositivo_id, db_id: d.id }
      })
      ...
      setLoteRows(mapeados.map(s => ({ id_logico: s.id, tipo_metrica: s.metric, valor: '', unidad: s.unit })))
  ...
  useEffect(() => {
    if (sensorSel) {
      ...
      if (activeTab === 'promedios') loadPromedios(sensorSel, ventana)
    }
  }, [sensorSel])
  ...
  const buscarParcelaDelSensor = async (id_logico) => {
    try {
      const res = await parcelasAPI.listar()
      for (const parcela of res.data) {
  ...
  const generarRecomendacionesAuto = async (sensorData, resultado) => {
    ...
    } catch(e) { console.warn('Recomendaciones auto no generadas:', e) }
  }
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (dispositivo && dispositivo.estado !== 'activo') {
          setAutoSim(false); setSimMsg(`Detenido — sensor ${sensorSel.id} ya no esta activo`); return
        }
      } catch(e) {}
      const val = parseFloat((Math.random() * (sensorSel.max - sensorSel.min) + sensorSel.min).toFixed(2))
      try {
        await ingestaAPI.enviarLectura({ dispositivo_id: 1, id_logico: sensorSel.id, tipo_metrica: sensorSel.metric, valor_metrica: val, unidad: sensorSel.unit })
        const resultado = await procesamientoAPI.procesarManual({ dispositivo_id: 1, id_logico: sensorSel.id, tipo_metrica: sensorSel.metric, valor_metrica: val, unidad: sensorSel.unit })
        if (resultado.data.alertas_generadas > 0) {
          const tipos = resultado.data.tipos_alerta
          await notificacionesAPI.enviar({ dispositivo_id: 1, id_logico: sensorSel.id, tipo_alerta: tipos[0], tipo_metrica: sensorSel.metric, valor: val, condicion: `Valor ${val} ${sensorSel.unit} — ${tipos.join(', ')}`, severidad: resultado.data.alertas_generadas > 1 ? 'critica' : 'alta', canal: 'push' })
          await generarRecomendacionesAuto({ id_logico: sensorSel.id, tipo_metrica: sensorSel.metric, valor_metrica: val }, resultado)
        }
        await loadLecturas(sensorSel); await loadAlertas(sensorSel)
      } catch(e) { console.error(e) }
    }, 5000)
  ...
  const handleSimular = async () => {
    ...
    try {
      const disps = await dispositivosAPI.listar(0, 200)
      const dispositivo = disps.data.find(d => d.id_logico === sensorSel.id)
      if (dispositivo && dispositivo.estado !== 'activo') { setSimMsg(`Sensor ${sensorSel.id} en estado "${dispositivo.estado}" — no puede recibir lecturas`); return }
    } catch(e) {}
    try {
      await ingestaAPI.enviarLectura({ dispositivo_id: 1, id_logico: sensorSel.id, tipo_metrica: sensorSel.metric, valor_metrica: val, unidad: sensorSel.unit })
      const resultado = await procesamientoAPI.procesarManual({ dispositivo_id: 1, id_logico: sensorSel.id, tipo_metrica: sensorSel.metric, valor_metrica: val, unidad: sensorSel.unit })
    ...
    } catch(e) { setSimMsg('Error enviando lectura'); console.error(e) }
  }
  ...
  const handleEnviarLote = async () => {
    ...
    try {
      for (const fila of filas) {
        ...
        await ingestaAPI.enviarLectura({ dispositivo_id: 1, id_logico: fila.id_logico, tipo_metrica: fila.tipo_metrica, valor_metrica: val, unidad: fila.unidad })
        const resultado = await procesamientoAPI.procesarManual({ dispositivo_id: 1, id_logico: fila.id_logico, tipo_metrica: fila.tipo_metrica, valor_metrica: val, unidad: fila.unidad })
        if (resultado.data.alertas_generadas > 0) {
          alertasTotal += resultado.data.alertas_generadas
          const tipos = resultado.data.tipos_alerta
          await notificacionesAPI.enviar({ dispositivo_id: 1, id_logico: fila.id_logico, tipo_alerta: tipos[0], tipo_metrica: fila.tipo_metrica, valor: val, condicion: `Valor ${val} ${fila.unidad} — ${tipos.join(', ')}`, severidad: resultado.data.alertas_generadas > 1 ? 'critica' : 'alta', canal: 'push' })
          await generarRecomendacionesAuto({ id_logico: fila.id_logico, tipo_metrica: fila.tipo_metrica, valor_metrica: val }, resultado)
        }
      }
      setLoteMsg(`${filas.length} lecturas enviadas · ${alertasTotal} alertas`)
      setLoteRows(prev => prev.map(r => ({ ...r, valor: '' })))
      if (sensorSel) setTimeout(() => { loadLecturas(sensorSel); loadAlertas(sensorSel) }, 500)
    } catch(e) { setLoteMsg('Error enviando lote') }
    finally { setLoteLoading(false); setTimeout(() => setLoteMsg(''), 6000) }
  }
    ...
    <div style={styles.wrapper} className="animate-fade">
      <div style={styles.header}>
        ...
        <div style={styles.headerRight}>
          ...
          <button onClick={() => setAutoSim(!autoSim)} style={{ ...styles.autoBtn, background: autoSim ? 'rgba(34,197,94,0.15)' : 'rgba(107,114,128,0.1)', color: autoSim ? '#22c55e' : '#6b7280', borderColor: autoSim ? 'rgba(34,197,94,0.3)' : 'rgba(107,114,128,0.2)' }}>
      ...
      <div style={styles.tabsBar}>
        {[
          ...
          { id: 'promedios', label: 'Promedios TimescaleDB' },
          { id: 'lote',      label: `Lote (${sensores.length} sensores)` },
        ].map(t => (
      ...
      {/* Grid de sensores */}
      {loadingSensores ? (
        <div style={styles.loadingGrid}>{[...Array(6)].map((_, i) => <div key={i} className="skeleton" style={{ height: '56px', borderRadius: '10px' }} />)}</div>
      ) : sensores.length === 0 ? (
        ...
        <div style={styles.sensorGrid}>
          {sensores.map(s => (
            <button key={s.id} onClick={() => setSensorSel(s)} style={{ ...styles.sensorBtn, borderColor: sensorSel?.id === s.id ? s.color : 'rgba(34,197,94,0.1)', background: sensorSel?.id === s.id ? `${s.color}15` : '#0d1510', color: sensorSel?.id === s.id ? s.color : '#6b7280' }}>
          ...
          <div className="card" style={{ marginBottom: '20px' }}>
            ...
            {loading ? (
              ...
              <div style={styles.noData}>
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#374151" strokeWidth="1.5" style={{ marginBottom: '8px' }}><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
          ...
          <div style={styles.bottomGrid}>
            <div className="card" style={{ flex: 1 }}>
              ...
              <div style={styles.simForm}>
                <input type="number" placeholder={`Valor (${sensorSel.min}–${sensorSel.max} ${sensorSel.unit})`} value={simVal} onChange={e => setSimVal(e.target.value)} style={styles.simInput} onKeyDown={e => e.key === 'Enter' && handleSimular()} />
                <button onClick={handleSimular} className="btn btn-primary">Enviar</button>
              </div>
              {simMsg && <div style={{ marginTop: '10px', fontSize: '13px', color: simMsg.includes('alerta') ? '#fbbf24' : simMsg.includes('Error') || simMsg.includes('estado') ? '#f87171' : '#22c55e' }}>{simMsg}</div>}
            </div>
            ...
            <div className="card" style={{ flex: 1 }}>
              ...
              {alertasFiltradas.length === 0 ? (
                <div style={styles.noAlertas}>
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#374151" strokeWidth="1.5" style={{ marginBottom: '6px' }}><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
      ...
      {/* ── TAB: Promedios TimescaleDB ── */}
      {activeTab === 'promedios' && (
        <div className="animate-fade">
          {!sensorSel ? (
            <div style={styles.noSensores}><div>Selecciona un sensor para ver sus promedios</div></div>
          ) : (
            <>
              <div className="card" style={{ marginBottom: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px' }}>Agregaciones por ventana de tiempo usando TimescaleDB</div>
                  </div>
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                    {VENTANAS.map(v => (
                      <button key={v.id} onClick={() => setVentana(v.id)} style={{ ...styles.ventanaBtn, ...(ventana === v.id ? styles.ventanaBtnActive : {}) }}>
                        {v.label}
                      </button>
                    ))}
                  </div>
              ...
              ) : promediosChart.length === 0 ? (
                <div style={styles.noSensores}>
                  <div>Sin datos en la ventana seleccionada</div>
                  <div style={{ fontSize: '11px', marginTop: '4px', color: '#4b5563' }}>Envía lecturas desde el simulador para ver promedios</div>
                </div>
              ) : (
                <>
                  {metricas_unicas.map(metrica => {
                    const datos  = promediosChart.filter(p => p.metrica === metrica)
              ...
              <div className="card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    <select value={diasExport} onChange={e => setDiasExport(Number(e.target.value))} style={styles.sevSelect}>
          ...
          <div className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '18px' }}>
              <div>
                ...
                <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px' }}>Deja en blanco los sensores que no quieras enviar.</div>
              </div>
            ...
            {loteRows.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px', color: '#4b5563', fontSize: '13px' }}>No hay sensores disponibles.</div>
            ) : (
              <>
                  ...
                  {loteRows.map((row, i) => {
                    ...
                    return (
                      <>
                        ...
                        <div key={`metric-${i}`} style={styles.loteCell}><span style={{ fontSize: '11px', color: '#9ca3af' }}>{row.tipo_metrica}</span></div>
                        <div key={`val-${i}`} style={styles.loteCellInput}>
                          <input type="number" placeholder="—" value={row.valor} onChange={e => setLoteRows(prev => prev.map((r, idx) => idx === i ? { ...r, valor: e.target.value } : r))} style={styles.loteInput} />
                        </div>
                        <div key={`unit-${i}`} style={styles.loteCell}><span style={{ fontSize: '12px', color: '#6b7280' }}>{row.unidad}</span></div>
                      </>
                    )
                  })}
                </div>
                <div style={{ marginTop: '18px', display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  <button onClick={() => setLoteRows(prev => prev.map(r => ({ ...r, valor: '' })))} className="btn btn-ghost" style={{ fontSize: '12px' }}>Limpiar</button>
                  {loteMsg && <span style={{ fontSize: '13px', color: loteMsg.includes('Error') ? '#f87171' : '#22c55e' }}>{loteMsg}</span>}
                </div>
              </>
            )}
          </div>
