...
export default function Dispositivos() {
  ...
  const [form, setForm] = useState({
    tipo_dispositivo_id: 1, numero_serial: '',
    id_logico: '', version_firmware: '1.0.0', estado: 'activo',
  })
  ...
  const handleRegistrar = async (e) => {
    e.preventDefault()
    if (!form.numero_serial || !form.id_logico) { setFormMsg('Completa serial e ID logico'); return }
    ...
    } catch(e) { setFormMsg('❌ Error al registrar') }
        ...
        <div style={styles.tableCard}>
          {loading ? (
            ...
            <div className="table-wrap">
              <table>
                <thead>
                  <tr><th>ID Logico</th><th>Tipo</th><th>Categoria</th><th>Serial</th><th>Estado</th><th>Firmware</th></tr>
                </thead>
                <tbody>
                  {filtered.map((d, i) => {
                    const tipo     = getTipo(d.tipo_dispositivo_id)
                    ...
                    return (
                      <tr key={d.id} onClick={() => handleSelect(d)} style={{ cursor: 'pointer', background: selected?.id === d.id ? 'rgba(34,197,94,0.08)' : 'transparent', animation: `fadeIn 0.3s ease ${i * 0.02}s both` }}>
                        ...
                        <td><span style={{ ...styles.catBadge, color: catStyle.color, background: catStyle.bg, border: `1px solid ${catStyle.border}` }}>{tipo?.categoria || '—'}</span></td>
          ...
          <div style={styles.detailPanel} className="animate-slide">
                ...
                {metricas && (
                  <div style={styles.metricasCard}>
                    ...
                    <div style={styles.metricasList}>
                      {metricas.metricas_permitidas?.map(m => <span key={m} style={styles.metricaTag}>{m}</span>)}
                    </div>
                  </div>
                )}
                {(() => {
            ...
            {activeTab === 'editar' && (
              <div style={styles.editPanel}>
                ...
                {editMsg && <div style={{ marginTop: '10px', fontSize: '13px', color: editMsg.startsWith('✅') ? '#22c55e' : '#f87171' }}>{editMsg}</div>}
              </div>
            )}
          </div>
        )}
