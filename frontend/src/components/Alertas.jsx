...
export default function Alertas() {
  ...
  const load = async () => {
    try {
      const [aRes, rRes, eRes] = await Promise.allSettled([
        procesamientoAPI.alertas(
          filtroSev !== 'todos' ? filtroSev : null,
          filtroTipo || null
        ),
        procesamientoAPI.resumen(),
        procesamientoAPI.eventos(undefined, 30),
      ])
      if (aRes.status === 'fulfilled') setAlertas(aRes.value.data)
      if (rRes.status === 'fulfilled') setResumen(rRes.value.data)
      if (eRes.status === 'fulfilled') setEventos(eRes.value.data)
  ...
  const TABS = [
    ...
    { id: 'eventos',  label: 'Eventos procesados'},
    ...
    <div style={styles.wrapper} className="animate-fade">
      ...
      <div style={styles.summaryGrid}>
        {[
          ...
          { label: 'Eventos',       val: resumen?.total_eventos || 0, color: '#a78bfa', bg: 'rgba(167,139,250,0.08)' },
        ].map(item => (
        ...
        <>
          ...
          {loading ? (
            ...
            <div style={styles.empty}>
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#374151" strokeWidth="1.5" style={{ marginBottom: '12px' }}>
                <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/>
                <polyline points="22 4 12 14.01 9 11.01"/>
              </svg>
              <div style={styles.emptyText}>Sin alertas activas</div>
              <div style={styles.emptySub}>El sistema opera dentro de los parametros normales</div>
            </div>
          ) : (
                  ...
                  <div key={a.id} style={{
                    ...
                    <div style={styles.alertLeft}>
                      <div style={styles.alertTopRow}>
                        <span style={{ ...styles.sevTag, color: sev.color, background: sev.bg, border: `1px solid ${sev.border}` }}>{sev.label}</span>
                    ...
                    <div style={styles.alertRight}>
                      ...
                      <div style={styles.alertUnidad}>{a.tipo_metrica}</div>
                    </div>
                  </div>
                )
              })}
      ...
      {activeTab === 'eventos' && (
        <div>
          <div style={{ marginBottom: '16px', display: 'flex', gap: '10px' }}>
            <div style={styles.countTag}>Ultimos {eventos.length} eventos procesados</div>
          </div>
          {eventos.length === 0 ? (
            <div style={styles.empty}>
              <div style={styles.emptyText}>Sin eventos procesados</div>
              <div style={styles.emptySub}>Genera datos desde el Dashboard o Lecturas</div>
            </div>
          ) : (
            <div className="table-wrap" style={{ background: '#0d1510', borderRadius: '16px', border: '1px solid rgba(34,197,94,0.1)', overflow: 'hidden' }}>
              <table>
                <thead>
                  <tr>
                    <th>ID</th><th>Sensor</th><th>Metrica</th>
                    <th>Valor</th><th>Alerta</th><th>Procesado</th>
                  </tr>
                </thead>
                <tbody>
                  {eventos.map((e, i) => (
                    <tr key={e.id} style={{ animationDelay: `${i * 0.02}s` }} className="animate-fade">
                      <td style={{ color: '#4b5563', fontSize: '11px' }}>#{e.id}</td>
                      <td><span style={{ fontFamily: 'monospace', color: '#4ade80', fontSize: '12px' }}>{e.id_logico}</span></td>
                      <td style={{ color: '#9ca3af', fontSize: '12px' }}>{e.tipo_metrica}</td>
                      <td style={{ color: '#f0fdf4', fontWeight: 600 }}>{e.valor_metrica}</td>
                      <td>
                        {e.tiene_alerta ? (
                          <span style={{ color: '#f87171', fontSize: '11px', fontWeight: 600 }}>Con alerta</span>
                        ) : (
                          <span style={{ color: '#22c55e', fontSize: '11px' }}>Normal</span>
                        )}
                      </td>
                      <td style={{ color: '#4b5563', fontSize: '11px' }}>{new Date(e.procesado_en).toLocaleString('es-CO')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {activeTab === 'resumen' && resumen && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={styles.resumenGrid}>
            {[
              { label: 'Total eventos procesados', val: resumen.total_eventos,    color: '#60a5fa' },
              { label: 'Total alertas generadas',  val: resumen.total_alertas,    color: '#f0fdf4' },
              { label: 'Alertas criticas',          val: resumen.alertas_criticas, color: '#f87171' },
              { label: 'Alertas altas',             val: resumen.alertas_altas,    color: '#fbbf24' },
              { label: 'Alertas medias',            val: resumen.alertas_medias,   color: '#60a5fa' },
            ].map(item => (
              <div key={item.label} style={{ background: '#0d1510', border: '1px solid rgba(34,197,94,0.1)', borderRadius: '14px', padding: '20px', textAlign: 'center' }}>
                <div style={{ fontFamily: "'Syne', sans-serif", fontSize: '36px', fontWeight: 800, color: item.color, lineHeight: 1 }}>{item.val}</div>
                <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '6px' }}>{item.label}</div>
              </div>
            ))}
          </div>

          <div className="card">
            <div style={styles.chartTitle}>Distribucion de alertas por severidad</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '8px' }}>
              {[
                { label: 'Criticas', val: resumen.alertas_criticas, color: '#f87171', total: resumen.total_alertas },
                { label: 'Altas',    val: resumen.alertas_altas,    color: '#fbbf24', total: resumen.total_alertas },
                { label: 'Medias',   val: resumen.alertas_medias,   color: '#60a5fa', total: resumen.total_alertas },
              ].map(item => (
                <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ width: '70px', fontSize: '12px', color: item.color, fontWeight: 600 }}>{item.label}</div>
                  <div style={{ flex: 1, height: '10px', background: 'rgba(255,255,255,0.05)', borderRadius: '5px', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${item.total ? (item.val / item.total * 100) : 0}%`, background: item.color, borderRadius: '5px', transition: 'width 0.8s ease' }} />
                  </div>
                  <div style={{ width: '30px', fontSize: '13px', fontWeight: 700, color: item.color, textAlign: 'right' }}>{item.val}</div>
                </div>
              ))}
            </div>
          </div>
...
const styles = {
  ...
  summaryGrid: { display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px', marginBottom: '20px' },
  ...
  alertCard: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 18px', borderRadius: '12px', background: '#0d1510', border: '1px solid rgba(34,197,94,0.08)' },
  alertLeft: { flex: 1 },
  ...
  resumenGrid: { display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px' },
  chartTitle: { fontFamily: "'Syne', sans-serif", fontSize: '14px', fontWeight: 600, color: '#f0fdf4', marginBottom: '12px' },
