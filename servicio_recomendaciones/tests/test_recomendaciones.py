...
def test_health():
    ...
    assert res.json()["servicio"] == "recommendation-engine"
...
def test_listar_categorias():
    ...
    assert "Proteccion" in nombres
    assert "Clima"      in nombres


# ── Generar recomendaciones ───────────────────────────
def test_generar_suelo_seco():
    res = client.post("/api/v1/recomendaciones/generar", json={
        "parcela_id":       1,
...
def test_generar_ph_acido():
    ...
    data = res.json()
    assert data["total_generadas"] >= 1
    titulos = [r["titulo"] for r in data["recomendaciones"]]
...
def test_listar_recomendaciones_vacias():
    res = client.get("/api/v1/recomendaciones/")
    assert res.status_code == 200
    assert isinstance(res.json(), list)


def test_listar_recomendaciones_con_datos():
    gen = client.post("/api/v1/recomendaciones/generar", json={
        "humedad_suelo": 15.0, "temperatura_aire": 30.0,
        "area_hectareas": 5.0, "tipo_cultivo": "maiz",
    })
...
def test_obtener_recomendacion():
    ...
    res = client.get(f"/api/v1/recomendaciones/{rec_id}")
    assert res.status_code == 200
    assert res.json()["id"] == rec_id


def test_obtener_recomendacion_inexistente():
    res = client.get("/api/v1/recomendaciones/9999")
    assert res.status_code == 404


def test_crear_recomendacion_manual():
    res = client.post("/api/v1/recomendaciones/", json={
        "categoria_id": 1,
        "parcela_id":   1,
        "titulo":       "Aplicar fertilizante NPK",
        "descripcion":  "El cultivo requiere nutricion adicional segun analisis foliar",
        "accion":       "Aplicar 50 kg/ha de NPK 15-15-15 en la proxima semana",
        "prioridad":    "media",
        "fuente":       "manual",
    })
    assert res.status_code == 201
    data = res.json()
    assert data["titulo"] == "Aplicar fertilizante NPK"
...
def test_actualizar_estado():
    ...
    assert gen.status_code == 201
    rec_id = gen.json()["recomendaciones"][0]["id"]
    res = client.put(f"/api/v1/recomendaciones/{rec_id}/estado", json={
        "estado": "aplicada"
    })
    assert res.status_code == 200
...
def test_recomendaciones_por_parcela():
    gen = client.post("/api/v1/recomendaciones/generar", json={
        "parcela_id": 5, "humedad_suelo": 15.0,
        "area_hectareas": 5.0,
    })
    assert gen.status_code == 201
    res = client.get("/api/v1/recomendaciones/parcela/5")
    assert res.status_code == 200
    assert len(res.json()) >= 1
    for r in res.json():
...
def test_recomendaciones_por_sensor():
    gen = client.post("/api/v1/recomendaciones/generar", json={
        "id_logico": "SOIL_HUM_02", "humedad_suelo": 12.0,
        "area_hectareas": 3.0,
    })
    assert gen.status_code == 201
    res = client.get("/api/v1/recomendaciones/sensor/SOIL_HUM_02")
    assert res.status_code == 200
    assert len(res.json()) >= 1
...
def test_recomendaciones_activas():
    gen = client.post("/api/v1/recomendaciones/generar", json={
        "humedad_suelo": 10.0, "temperatura_aire": 28.0,
        "area_hectareas": 5.0,
    })
    assert gen.status_code == 201
    res = client.get("/api/v1/recomendaciones/activas")
    assert res.status_code == 200
    for r in res.json():
...
def test_resumen():
    ...
    assert gen.status_code == 201
    res = client.get("/api/v1/recomendaciones/resumen")
    assert res.status_code == 200
    data = res.json()
