...
def inicializar_timescale():
    try:
        with engine.connect() as conn:
            ...
            """)).scalar()
            if result == 0:
                conn.execute(text("ALTER TABLE lectura_sensor DROP CONSTRAINT IF EXISTS lectura_sensor_pkey;"))
                conn.execute(text("""
                ...
                conn.execute(text("""
                    SELECT create_hypertable(
                        'lectura_sensor', 'timestamp_lectura',
...
@app.get("/health")
def health_check():
    return {"estado": "ok", "servicio": "iot-ingestion", "version": "1.0.0", "timescale": "activo"}